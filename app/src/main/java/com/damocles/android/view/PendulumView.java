...
public class PendulumView extends View {
    ...
    private void rotate(Canvas canvas) {
        ...
        RectF rectF = new RectF(topPoint[0] - totalLength, topPoint[1] - totalLength, topPoint[0] + totalLength,
                topPoint[1] + totalLength);
        //绘制1/4个圆的弧形
        bigCirclePath.addArc(rectF, 90, -90 * direction);
        //用来确定旋转nowDegree时的边界坐标；
        float[] rotatePoint = new float[2];
        PathMeasure measure = new PathMeasure(bigCirclePath, false);        //此时，rotatePoint的坐标就为我们图中小圆圈圆心的坐标
